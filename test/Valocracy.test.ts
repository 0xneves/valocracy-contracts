...
describe("Valocracy", function () {
	...
	// TODO: Move to a different file TestOwnership.test.ts
	describe("Test Ownership function", function () {});

	// TODO: Move to a different file TestERC721.test.ts
	...
	describe("SetUp Valocracy", function () {
		...
		it("Should create a new Valor", async function () {
			...
			await expect(await Valocracy.setValor(valorId, valorRarity, valorMeta))
	...
	describe("Test Valocracy Implementation", function () {
		...
		it("Should fail to transfer the token after its been minted", async function () {
			await expect(Valocracy.connect(Jhon).transferFrom(Jhon.address, Alice.address, 1))
		...
		it("Should be able to transfer the token to the zero address", async function () {
			...
			.to.emit(Valocracy, "Transfer").withArgs(Jhon.address, ethers.constants.AddressZero, 1);
		});
