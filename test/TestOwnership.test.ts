...
describe("Valocracy", function () {
    ...
    describe("Test Ownership function", function () {
        ...
        it("Should only owner can setValor", async function () {
            ...
            await expect(
                Valocracy.connect(Jhon).setValor(
                    0,
                    1000,
                    "ipfs://QmeSjSinHpPnmXmspMjwiXyN6zS4E9zccariGR3jxcaWtq/1",
                ),
            ...
            await expect(
                await Valocracy.connect(deployer).setValor(
                    valorId,
                    valorRarity,
                    valorMeta,
                ),
        ...
        it("Should only owner can mint", async function () {
            await expect(
                Valocracy.connect(Jhon).mint(Jhon.address, 0),
            ).to.be.revertedWith("Ownable: caller is not the owner");

            await expect(
                await Valocracy.connect(deployer).mint(Jhon.address, 0),
            )
